...
from lhorizon.lhorizon_utils import hat, utc_to_et, unit_vectors
from lhorizon.target import array_reference_shift
...
def clipped_function_factory(
    input_function, lower_bound=None, upper_bound=None
):
...
def phong_moon(
    ...
    observer_selenocenter_query = LHorizon(
        301, epochs=tdb_time, location=observer_location
    )
    observer_sun_query = LHorizon(
...
def microfacet_moon(
    ...
    return [
        ...
        for normal, observer, illumination in zip(
            unit_normal_vectors,
            unit_observer_vectors,
            unit_illumination_vectors,
        )
    ]
